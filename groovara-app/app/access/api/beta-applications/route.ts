...
const listeningStylePrefix = "Listening style — ";
const sharingReasonPrefix = "Sharing reason — ";

const allowedGoals = new Set([
  ...allowedListeningStyles.map(
    (answer) => `${listeningStylePrefix}${answer}`,
  ),
  ...allowedSharingReasons.map(
    (answer) => `${sharingReasonPrefix}${answer}`,
  ),
]);
...
export async function POST(request: Request) {
  const body = (await request
    .json()
    .catch(() => null)) as SubmissionBody | null;
  ...
  const collectionHistory = cleanStringArray(
    body.collectionHistory,
    allowedHistory,
  );

  const collectionGoals = cleanStringArray(
    body.collectionGoals,
    allowedGoals,
  );

  const collectionGoalOther =
    cleanString(body.collectionGoalOther, 240) || null;

  const musicServices = cleanStringArray(
    body.musicServices,
    allowedServices,
  );

  const musicServiceOther =
    cleanString(body.musicServiceOther, 240) || null;
  ...
  if (!allowedDecades.has(lifeDecade)) {
    return NextResponse.json(
      { error: "Choose an age range." },
      { status: 400 },
    );
  }

  if (
    !Number.isInteger(musicMeaning) ||
    musicMeaning < 1 ||
    musicMeaning > 10
  ) {
    return NextResponse.json(
  ...
  const hasListeningStyle = collectionGoals.some((goal) =>
    goal.startsWith(listeningStylePrefix),
  );

  const hasSharingReason = collectionGoals.some((goal) =>
    goal.startsWith(sharingReasonPrefix),
  );
  ...
  if (error) {
    if (error.code === "23505") {
      return NextResponse.json(
        {
          error:
            "A beta request already exists for this email address.",
        },
        { status: 409 },
      );
    }
